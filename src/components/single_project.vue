<template>
    <section class="min-h-screen flex flex-col items-center justify-center bg-zinc-900 text-zinc-300 p-6">
        <div class="max-w-7xl w-full text-left">
            <img v-if="project" :src="project.image" alt="Project image" class="w-full max-h-96 object-cover rounded-md shadow-md mb-6" />
            <h1 class="sm:text-4xl text-2xl font-bold text-accent mb-4">{{ project?.title || 'Project Not Found' }}</h1>
            <p class="sm:text-lg text-sm text-zinc-300 mb-6">{{ project?.description || 'This project does not exist.' }}</p>
            <div v-if="project?.technologies?.length" class="mb-6">
                ...
                <div class="flex flex-wrap gap-2 justify-left sm:justify-start">
            ...
            <div v-if="project?.liveLink || project?.githubLink" class="mt-6 w-full p-4 text-left rounded-md sm:p-8 bg-gradient-to-r from-zinc-800 via-zinc-900 to-zinc-900">
                <h2 class="sm:text-2xl text-xl font-semibold text-zinc-200 mb-4 text-left">
